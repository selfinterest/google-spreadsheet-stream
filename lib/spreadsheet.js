...
function parseQueryParameters(parameters){
    ...
    if(parameters.limit){
        query += "max-results=" + parameters.limit;
    }
    ...
    if(parameters.offset){
        ...
        query += "start-index=" + parameters.offset;
    }
    ...
    if(parameters.query){
        ...
        query = "sq=" + encodeURIComponent(parameters.query);
...
Spreadsheet.prototype.baseUrl = function() {
	...
	if(!this.offset && !this.limit && !this.query) {
		return url;
	} else {
    var query = parseQueryParameters(this);
...
Spreadsheet.prototype.metaUrl = function(){
	var url =  this.protocol + "://spreadsheets.google.com/feeds/worksheets/" + this.spreadsheetId + "/private/full/" + this.worksheetId;
