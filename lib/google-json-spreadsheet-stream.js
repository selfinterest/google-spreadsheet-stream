...
function GoogleJsonSpreadsheetStream(optionsObject, spreadsheetPromise, streamOptions) {
	if (this instanceof GoogleJsonSpreadsheetStream) {
      ...
      /*var gotSpreadsheetAndGotToken = Q.all([spreadsheetPromise, tokenPromise]);

    var gotSpreadsheetTokenAndMetadata = gotSpreadsheetAndGotToken.spread(function(spreadsheet, token){
        console.log("Getting metadata");
        return getMetaDataStream(spreadsheet, token);
    });


    gotSpreadsheetTokenAndMetadata.then(function(unparsedMetadataStream){
        console.log("Receiving metadata");
        this.metadataStream = new XmlStream(this.unparsedMetadataStream);

        this.metadataStream.on("data", function(data){
            console.log(data);
        })
    });
    ...
    //this.metadataStream = new XmlStream(this.metadataStream);   //parse the stream
*/
